...
if sudo docker ps -a | grep -q $DATABASE_DOCKER_CONTAINER; then
    echo "There are running containers. Deleting only associated containers."
    sudo docker rm -f $DATABASE_DOCKER_CONTAINER
    sudo docker run --name $DATABASE_DOCKER_CONTAINER -e POSTGRES_PASSWORD=home -e POSTGRES_DB=$POSTGRES_DB -d -p $POSTGRES_HOST:$POSTGRES_HOST $POSTGRES_USER
else
...
DATABASE_DOCKER_CONTAINER_IP_ADDRESS=$(sudo docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $DATABASE_DOCKER_CONTAINER)

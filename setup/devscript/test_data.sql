...
INSERT INTO community.projects
    (title, description, street, city, state, zip, lat, lon, cause, project_type, max_attendees, attendees, required_total_man_hours, deactivated, start_date, collaborators, created_by, updated_by, sharable_groups)
VALUES
    ('SC Go raiders', 'South Carolina Football Team Inventory List', '5 S Mill St', 'Manning', 'SC', '29102', '33.75', '-80.22', 'Community Cause', 'Test Project Type', floor(random()*(10-2)+2), array[(SELECT id FROM profiles LIMIT 1)::UUID], 250, false, NOW(), array[(SELECT id FROM profiles LIMIT 1)::UUID], (SELECT id FROM community.profiles LIMIT 1), (SELECT id FROM community.profiles LIMIT 1), array[(SELECT id FROM profiles LIMIT 1)::UUID]),
    ('North Carolina Football Team', 'North Carolina football team', NULL, NULL, NULL, NULL, '35.12', '-78.22','Environment Development', 'Test Project Type', floor(random()*(10-2)+2), array[(SELECT id FROM community.profiles LIMIT 1)::UUID], 250, false, NOW(), array[(SELECT id FROM profiles LIMIT 1)::UUID], (SELECT id FROM community.profiles LIMIT 1), (SELECT id FROM community.profiles LIMIT 1), array[(SELECT id FROM community.profiles LIMIT 1)::UUID]),
    ('Worthington Football Team', 'Worthington Football Team', NULL, NULL, NULL, NULL, '39.96', '-82.99', 'Community Cause', 'Community Development', floor(random()*(10-2)+2), array[(SELECT id FROM community.profiles LIMIT 1)::UUID], 250, true, NOW(), array[(SELECT id FROM profiles LIMIT 1)::UUID], (SELECT id FROM community.profiles LIMIT 1), (SELECT id FROM community.profiles LIMIT 1), array[(SELECT id FROM community.profiles LIMIT 1)::UUID]),
    ('Austin Zoo Fair Show', 'Austin zoo fair show for children and adults', 'S Congress Ave', 'Austin', 'TX', '78702', '30.26', '-97.74', 'Community Cause', 'Community Development', floor(random()*(10-2)+2), array[(SELECT id FROM community.profiles LIMIT 1)::UUID], 250, false, NOW(), array[(SELECT id FROM profiles LIMIT 1)::UUID], (SELECT id FROM community.profiles LIMIT 1), (SELECT id FROM community.profiles LIMIT 1), array[(SELECT id FROM community.profiles LIMIT 1)::UUID]),

...
//
// GenerateFakeDataWithLimit retrieves fake data for assetTracker application with a limit of rows.
...
func SetupDB(user string) (*sql.DB, error) {
	...
	psqlStr := fmt.Sprintf("postgres://%s:%s@%s:%s/asset?sslmode=disable", user, pwd, host, port)
